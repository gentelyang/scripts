        ...
        for place in self.places:
            ...
            if self.enable_backward:
                ...
                for i in range(10):
                    ...
                    forward_time = forward_end_time - forward_begin_time
                    print(" ===============================>>>>>>>>forward_time", forward_time)
                    forward_sum_time.append(forward_time)
                    print(static_forward_res)
#                    compare(static_forward_res, res, self.delta, self.rtol)
            avg_forward_time = np.sum(forward_sum_time[1:]) / 9
            print(" ===============================>>>>>>>>avg_forward_time is: ", avg_forward_time)
            print(" ===============================>>>>>>>>all_forward_time is: ", forward_sum_time[1:])
            print(" ===============================>>>>>>>>sum_forward_time is: ", np.sum(forward_sum_time[1:]))
            if self.enable_backward:
                # 静态图这个地方怎么compute grad，使用了动态图的计算梯度的方法计算得到grad，然后和动态图进行对比？
                print("+++++++++++++++++++++++++")
                grad = self.compute_grad(res, data, **kwargs)
                print("+++++++++++++++++++++++++", grad)
                compare_grad(static_backward_res, grad, mode="static", delta=self.delta, rtol=self.rtol,
