...
function build_app() {
  setproxy
 # pip2.7 install paddlepaddle paddlehub ujson paddle_serving_client Pillow
...
function faster_rcnn_model_rpc(){
  ...
  python2.7 -m paddle_serving_server_gpu.serve --model pddet_serving_model --port 8870 --gpu_id 0 > haha 2>&1 &
  sleep 3
