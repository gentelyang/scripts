#!/bin/env python
# -*- coding: utf-8 -*-
# encoding=utf-8 vi:ts=4:sw=4:expandtab:ft=python
#======================================================================
#
# Copyright (c) 2017 Baidu.com, Inc. All Rights Reserved
#
#======================================================================
"""
/***************************************************************************
  *
  * Copyright (c) 2019 Baidu.com, Inc. All Rights Reserved
  * @file test_dist_fleet_worker.py
  * @author liyang109@baidu.com
  * @date 2021-01-20 15:01
  * @brief 
  *
  **************************************************************************/
"""
import os
import subprocess


class TestFleetWorkerServerApi():
    """TestFleetWorkerServerApi"""
    def test_dist_fleet_worker_index(self):
        """test_dist_fleet_worker"""
        cmd = 'fleetrun dist_fleet_worker_index.py'
        pro = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = pro.communicate()
        pro.wait()
        pro.returncode == 0

    def test_dist_fleet_init_server(self):
        """test_dist_fleet_init_server"""
        cmd = 'fleetrun dist_fleet_init_server.py'
        pro = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = pro.communicate()
        pro.wait()
        pro.returncode == 0

    def test_dist_fleet_init_worker(self):
        """test_dist_fleet_init_worker"""
        cmd = 'fleetrun dist_fleet_init_worker.py'
        pro = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = pro.communicate()
        pro.wait()
        pro.returncode == 0

    def test_dist_fleet_is_first_worker(self):
        """test_dist_fleet_is_first_worker"""
        cmd = 'fleetrun dist_fleet_is_first_worker.py'
        pro = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = pro.communicate()
        pro.wait()
        pro.returncode == 0

    def test_dist_fleet_is_worker(self):
        """test_dist_fleet_is_worker"""
        cmd = 'fleetrun dist_fleet_is_worker.py'
        pro = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = pro.communicate()
        pro.wait()
        pro.returncode == 0

    def test_dist_fleet_worker_endpoints(self):
        """test_dist_fleet_worker_endpoints"""
        cmd = 'fleetrun dist_fleet_worker_endpoints.py'
        pro = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = pro.communicate()
        pro.wait()
        pro.returncode == 0

    def test_dist_fleet_is_server(self):
        """test_dist_fleet_is_server"""
        cmd = 'fleetrun dist_fleet_worker_is_server.py'
        pro = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = pro.communicate()
        pro.wait()
        pro.returncode == 0

    def test_dist_fleet_server_endpoints(self):
        """test_dist_fleet_server_endpoints"""
        cmd = 'fleetrun dist_fleet_worker_server_endpoints.py'
        pro = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = pro.communicate()
        pro.wait()
        pro.returncode == 0

    def test_dist_fleet_server_index(self):
        """test_dist_fleet_server_endpoints"""
        cmd = 'fleetrun dist_fleet_worker_server_index.py'
        pro = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = pro.communicate()
        pro.wait()
        pro.returncode == 0

    def test_dist_fleet_server_num(self):
        """test_dist_fleet_server_num"""
        cmd = 'fleetrun dist_fleet_worker_server_num.py'
        pro = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = pro.communicate()
        pro.wait()
        pro.returncode == 0
