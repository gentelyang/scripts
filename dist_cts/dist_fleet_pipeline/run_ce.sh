...
function _init_parameters(){
    ...
    with_pslib='OFF'
    with_mkl="ON"
    email_address=liyang109@baidu.com
}
_init_parameters
while [[ $# -gt 0 ]]; do
    case "$1" in
...
function build(){
    ...
    if [[ ${device_type} == 'cpu' || ${device_type} == "CPU" ]]; then
        ...
        IMAGE_NAME=paddlepaddle-0.0.0.${PADDLE_VERSION}-cp37-cp37m-linux_x86_64.whl
        with_gpu="OFF"
    ...
    if [[ ${with_pslib} == 'ON' ]]; then
        docker run -i --rm -v $PWD:/paddle \
            ...
            /bin/bash -c "mkdir -p /paddle/build && cd /paddle/build; pip install protobuf; \
                         ...
                         make -j$(nproc)"
            build_name="paddlepaddle-0.0.0-cp37-cp37m-linux_x86_64.whl"
